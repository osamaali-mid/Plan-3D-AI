...
CARPET_OUTPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "carpet_output")
...
app = FastAPI(lifespan=lifespan, title="Room 3D Design Suite", 
              description="API for floorplan recognition and carpet generation with modular architecture")
...
app.mount("/api/carpet/images", StaticFiles(directory=CARPET_OUTPUT_DIR), name="carpet_images")
...
# Import carpet generation routes
from app.carpet_generation.routes import router as carpet_router
from app.floorplan.routes import router as floorplan_router

# Include carpet generation routes
app.include_router(carpet_router)
